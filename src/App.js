...
const FileUpload = ({ onDataLoaded }) => {
  ...
  const handleMappingConfirmed = (mapping) => {
    ...
    const processedData = rawData.map(row => {
      ...
      if (mapping.productCode && mapping.productCode !== 'ignore') {
        processedRow._mappedProductCode = row[mapping.productCode];
      }
      
      if (mapping.department && mapping.department !== 'ignore') {
        processedRow._mappedDepartment = row[mapping.department];
      }
      
      if (mapping.dailyCubicOutput && mapping.dailyCubicOutput !== 'ignore') {
...
const ColumnMapper = ({ headers, rawData, onMappingConfirmed, onCancel }) => {
  ...
  }, [headers]); // Removido mapping e autoDetected das dependências para evitar loop
  ...
  const handleMappingChange = (field, value) => {
    ...
    setAutoDetected(prev => ({
      ...prev,
      [field]: false // Resetar detecção automática para este campo
    }));
  };
  ...
  const handleConfirm = () => {
    ...
    if (
      (mapping.productCode === '' || mapping.productCode === 'ignore') ||
      (mapping.department === '' || mapping.department === 'ignore') ||
      (mapping.dailyCubicOutput === '' || mapping.dailyCubicOutput === 'ignore')
    ) {
        ...
        {requiredFields.map((field) => (
          <div className="mapping-item" key={field.id}>
            ...
            >
              ...
              <option value="ignore">Não utilizar (ignorar)</option>
              {headers.map((header, index) => (
  ...
  const exportToExcel = () => {
    ...
    const exportData = data.map(item => {
      ...
      if (item._columnMapping) {
        ...
        if (mapping.description && mapping.description !== 'ignore' && item[mapping.description]) {
          exportRow['Descrição'] = item[mapping.description];
        }
        
        if (mapping.category && mapping.category !== 'ignore' && item[mapping.category]) {
          exportRow['Categoria'] = item[mapping.category];
        }
        
        if (mapping.supplier && mapping.supplier !== 'ignore' && item[mapping.supplier]) {
          exportRow['Fornecedor'] = item[mapping.supplier];
        }
        
        if (mapping.stock && mapping.stock !== 'ignore' && item[mapping.stock]) {
          exportRow['Estoque'] = item[mapping.stock];
        }
        
        if (mapping.weight && mapping.weight !== 'ignore' && item[mapping.weight]) {
          exportRow['Peso'] = item[mapping.weight];
        }
        
        if (mapping.dimensions && mapping.dimensions !== 'ignore' && item[mapping.dimensions]) {
          exportRow['Dimensões'] = item[mapping.dimensions];
        }
        
        if (mapping.price && mapping.price !== 'ignore' && item[mapping.price]) {
...
function App() {
  ...
  const processData = (items, params = parameters) => {
    ...
    const processed = items.map(item => {
      ...
      if (columnMapping && columnMapping.dailyCubicOutput && columnMapping.dailyCubicOutput !== 'ignore') {
        ...
        // Fallback para coluna padrão se não mapeado ou ignorado
        // Se dailyCubicOutput for 'ignore' ou não mapeado, saidaCubicaDia permanece 0
      }
