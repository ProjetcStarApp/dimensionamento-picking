...
const DetailedResults = ({ data }) => {
  ...
  const exportToExcel = () => {
    ...
    const exportData = data.map(item => ({
      ...
      'Saída Cúbica/Dia': item.saidaCubicaDia || 0,
  ...
  const filteredData = data.filter(item => {
    ...
    } else if (sortField === 'saidaCubicaDia') {
      valueA = a.saidaCubicaDia || 0;
      valueB = b.saidaCubicaDia || 0;
          ...
          <table className="data-table">
            <thead>
              <tr>
                ...
                <th onClick={() => handleSort('saidaCubicaDia')} className="sortable">
                  Saída Cúbica/Dia {sortField === 'saidaCubicaDia' && (sortDirection === 'asc' ? '▲' : '▼')}
                </th>
